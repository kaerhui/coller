...
std::vector <vector ChessPosition>chess_map; //游戏地图
bool IS_FIRSTHAND; //是否为先手
bool IS_COMMANER_VALUE;
bool IS_CHIEF_VALUE;
std::queue <ChessPosition> our_chess;
std::queue <ChessPosition> opp_chess;
...
public:
    ...
    int count_spt(std:: vector <vector Position> a) // 判断剩下棋子数
    {
        int num = 0 ;
        for()
            return  num;
    ...
    std::queue <ChessPosition> elvaute(std::)//打分函数
    {
        return ;
    }

    void find_pos(ChessPosition a,std :: queue <ChessPosition> &can_pos) //找到棋子目前可以走的点
    {
        int pos_x = a.position().x();
        int pos_y = a.position().y();

        if((pos_x==1 && pos_==9) ||
            (pos_x==1 && pos_x)==7)||
            (pos_x==1 && a.position().y()==4)||
            (pos_x==1 && a.position().y()==2)||
            (pos_x==3 && a.position().y()==9) ||
            (pos_x==3 && a.position().y()==7) ||
            (pos_x==3 && a.position().y()==4) ||
            (pos_x==3 && a.position().y()==2) ||
            (pos_x==2 && a.position().y()==3) ||
            (pos_x==2 && a.position().y()==8)
                        )//棋子在行营中
        {
            new Position a();
            a.position().set_x();
    ...
    int Find_chess(ChessPosition position)//找到可以走的棋子
    {
        std::queue <ChessPosition> move_queue;//可移动的路劲

    }

    void Chess_move(ChessPosition Now_Chess,std::queue <ChessPosition> Finally_Chess)
    {
        ChessPosition move_chess_postion ;//移动点
        for(;!Finally_Chess.empty();Finally_Chess.push())
        {
            move_chess_postion = Finally_Chess.push();
            Now_Chess.position().set_x(move_chess_postion.position().x());
            Now_Chess.position().set_y(move_chess_postion.position().y());
            response->add_position()->CopyFrom(Now_Chess);
            response->add_position()->CopyFrom(move_chess_postion);
        }
    }
    /**
     * 初始化棋盘
    ...
    Status move(ServerContext* context, const MoveRequest* request,
      ...
      //
        ...
        for(int i = 0 ; i<50; i++)//初始化地图，我方队列，和对方队列
        {
            chess = request->chesspositions(i);
            map[chess.position().y()][chess.position().x()]=chess;
            if(IS_FIRSTHAND)
            {
                for(i=1 ;i<26 ; i++)
                {
                    if(request->chesspositions(i).has_position())
                        our_chess.add(request->chesspositions(i));
                }
            }
            else
            {
        ...
        std :: queue < queue<ChessPosition> > move_queue;
        std :: queue<ChessPosition> move_chess_potion;
        for(;!our_chess.empty;our_chess.push())
        {
            find_pos(our_chess.push(),move_queue);
            move_chess_potion=elvaute(move_queue);
            Chess_move(our_chess.push(),move_chess_potion);
        }
